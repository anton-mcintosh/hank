...
async def generate_invoice_html(work_order, customer=None, vehicle=None, is_estimate=False):
    """
    ...
    try:
        ...
        html_filename = f"{document_type.lower()}_{work_order.id[:8]}_{timestamp}.html"
        ...
        if vehicle:
            ...
            vehicle_data = {
                ...
                "vin": vi.get("vin", "N/A"),
                "mileage": f"{vi.get('mileage', 'N/A'):,}" if vi.get("mileage") else "N/A"
            }
...
async def generate_pdf_with_reportlab(template_data, output_path=None):
    ...
    try:
        ...
        if not output_path:
            document_type = "estimate" if template_data.get("is_estimate", False) else "invoice"
            timestamp = datetime.now().strftime("%Y%m%d%H%M%S")
            unique_id = str(uuid.uuid4())[:8]
        ...
        styles.add(ParagraphStyle(name='CenterHeading', parent=styles['Heading1'], alignment=1, spaceAfter=12))
        styles.add(ParagraphStyle(name='RightAligned', parent=styles['Normal'], alignment=2))
        ...
        header_data = [
            [
                ...
                # Document info
                [Paragraph(f"<b>{template_data['document_type']}</b>", styles['RightAligned']),
                 Paragraph(f"<b>Number:</b> {template_data['order_id']}", styles['RightAligned']),
                 Paragraph(f"<b>Date:</b> {template_data['date']}", styles['RightAligned'])]
            ]
        ]
        ...
        vehicle_table = Table([
            [Paragraph("<b>Year:</b>", styles['Normal']), template_data['vehicle_year'],
             Paragraph("<b>Make:</b>", styles['Normal']), template_data['vehicle_make']],
            [Paragraph("<b>Model:</b>", styles['Normal']), template_data['vehicle_model'],
             Paragraph("<b>VIN:</b>", styles['Normal']), template_data['vehicle_vin']],
            [Paragraph("<b>Mileage:</b>", styles['Normal']), template_data['vehicle_mileage'], "", ""]
